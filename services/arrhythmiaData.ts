...
const IVR_QRS_VECTOR: Vector = { magnitude: 1.2, angle: -80, duration: 0.24, points: [[0,0], [0.15, 0.3], [0.3, 1.0], [0.5, -0.8], [0.75, 0.4], [1,0]]};
const IVR_T_VECTOR: Vector = { magnitude: 0.3, angle: 100, duration: 0.18, points: [[0,0], [0.5, -1], [1,0]]};
...
export const arrhythmias: Arrhythmia[] = [
  ...
  },
  {
    ...
    quiz: [
        ...
        { question: '¿Cuál es el principal riesgo de un bloqueo Mobitz II?', options: ['Progresión a fibrilación auricular', 'Progresión a bloqueo AV completo (3er grado)', 'Causar hipertensión severa', 'No tiene riesgos significativos'], correctAnswer: 1, explanation: 'El Mobitz II es considerado un bloqueo inestable y peligroso debido a su alto riesgo de progresar síbitamente a un bloqueo AV de tercer grado, lo que puede causar un paro cardíaco.'},
    ],
    generateECGData: (duration) => {
        let data: Record<string, ECGPoint[]> = Object.fromEntries(Object.keys(LEAD_ANGLES).concat(['V1','V2','V3','V4','V5','V6']).map(l => [l, []]));
        let time = 0;
  ...
  },
  {
    ...
    category: ArrhythmiaCategory.VENTRICULARES,
    description: 'Ritmo en el que cada tres latidos sinusales normales son seguidos por una Contracción Ventricular Prematura (CVP), creando un patrón repetitivo de "normal-normal-normal-ectópico".',
    criteria: { rhythm: 'Regularmente irregular', rhythmAnalysis: 'Patrón 3:1', rate: 'Depende del ritmo base', pWave: 'Presente en latidos sinusales', prInterval: 'Normal en latidos sinusales', qrs: 'Tres normales, uno ancho (CVP)' },
    approximateBpm: 75,
    clinicalSignificance: 'Similar al bigeminismo y trigeminismo, su importancia clínica depende del contexto (presencia de cardiopatía, síntomas, frecuencia).',
    nursingConsiderations: 'Evaluar los electrolitos del paciente y la oxigenación. Documentar la frecuencia del patrón y cualquier síntoma asociado.',
    emergencyProtocol: 'No es una emergencia por sí mismo. El tratamiento se dirige a la causa subyacente.',
    quiz: [
        { question: '¿Cuál es el patrón característico de la cuadrigeminia ventricular?', options: ['Normal, CVP, Normal, Normal...', 'Normal, Normal, Normal, CVP...', 'Normal, Normal, CVP, Normal...', 'Cuatro CVPs seguidas'], correctAnswer: 1, explanation: 'El patrón de cuadrigeminia es tres latidos normales seguidos de una CVP, que se repite.'},
        { question: 'En cuadrigeminia, ¿cada cuántos latidos ocurre una CVP?', options: ['Cada 2', 'Cada 3', 'Cada 4', 'Cada 5'], correctAnswer: 2, explanation: 'En cuadrigeminia, 1 de cada 4 latidos es una CVP (tres normales, uno ectópico).'},
        { question: '¿Cuándo la cuadrigeminia ventricular requiere tratamiento?', options: ['Siempre', 'Nunca', 'Si es sintomática o en contexto de cardiopatía estructural', 'Solo si la FC > 100'], correctAnswer: 2, explanation: 'Al igual que otros patrones de ectopia ventricular, el tratamiento se reserva para pacientes sintomáticos o con cardiopatía estructural subyacente.'},
    ],
    generateECGData: (duration) => {
        const bpm = 75;
        const normalInterval = 60 / bpm;
  ...
  },
  {
    ...
    generateECGData: (duration) => {
        ...
        while(time < duration) {
            ...
            const phase = time / (60 / avgBpm);
            const axisModulation = Math.sin(phase * 0.4 * Math.PI) * 60;
            const amplitudeModulation = 0.4 + Math.sin(phase * 0.2 * Math.PI) * 0.8 + 0.8;
            const modulatedQRS: Vector = {...VT_QRS_VECTOR, magnitude: amplitudeModulation, angle: -90 + axisModulation};
            const modulatedT: Vector = {...VT_T_VECTOR, magnitude: amplitudeModulation * 0.3, angle: 90 + axisModulation};
  ...
  },
  {
    ...
    generateECGData: (duration) => {
        ...
        let data: Record<string, ECGPoint[]> = {};
        for (let time = 0; time < duration; time += interval) {
            const beat = create12LeadBeat(time, null, IVR_QRS_VECTOR, IVR_T_VECTOR);
            for(const lead in beat) {
                if(!data[lead]) data[lead] = [];
                data[lead].push(...beat[lead]);
            }
  ...
  },
  {
    ...
    category: ArrhythmiaCategory.VENTRICULARES,
    description: 'Ritmo ventricular regular con una frecuencia entre 40 y 120 lpm. Es más rápido que un ritmo de escape ventricular pero mís lento que una taquicardia ventricular. A menudo se observa durante la reperfusión en un infarto de miocardio.',
